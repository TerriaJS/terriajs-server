...
function loadCommandLine() {
    var yargs = require('yargs')
        ...
        'multi-config-file' : {
            'description' : 'File containing mapping for multi-tenant mode. See multiserverconfig.json.example'
        },
        'proxy-auth' : {
...
options.init = function(quiet) {
    ...
    this.wwwroot = argv._.length > 0 ? argv._[0] : path.join(process.cwd(), 'wwwroot');
    ...
    this.multiConfigFile = getConfigFile(argv.multiConfigFile, 'multiserverconfig.json');
    this.multiConfig = getConfig(this.multiConfigFile, 'multi-config', 'multi-tenant config not supplied', quiet);
    return options;
};
