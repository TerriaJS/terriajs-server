 ...
 */
function getConfig(filePath, configFileType, failureConsequence, isMaster) {
    var config;
...
config.init = function(isMaster) {
    ...
    this.settings = getConfig(this.configFile, 'config', 'ALL proxy requests will be accepted.', isMaster);
    this.proxyAuthFile = getConfigFile(argv.proxyAuth, 'proxyauth.json');
    this.proxyAuth = getConfig(this.proxyAuthFile, 'proxyAuth', 'Proxying to servers that require authentication will fail', isMaster);
