...
module.exports = function(options) {
    ...
    app.get('*', function (req, res, next) {
        ...
        // Construct a partially user-provided path. Must check that the path doesn't attempt to escape wwwroot
        const prerendered = path.resolve(options.wwwroot, `prerendered/${decodedPath}`, 'index.html');
        if (!prerendered.startsWith(path.resolve(options.wwwroot, 'prerendered') + '/')) {
            if (options.verbose) {
                console.log('Request tried to escape prerendered path');
            }
        } else if (fs.existsSync(prerendered)) {
