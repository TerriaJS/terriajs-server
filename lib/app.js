...
// We do it after the /proxy service because it has its own authentication system (using `Proxy-Authenticate` instead of `WWW-Authenticate`).
if (configSettings.basicAuthentication && configSettings.basicAuthentication.username && configSettings.basicAuthentication.password) {
    app.use(basicAuth(configSettings.basicAuthentication.username, configSettings.basicAuthentication.password));
}
