...
if (configSettings.basicAuthentication && configSettings.basicAuthentication.username && configSettings.basicAuthentication.password) {
    app.use(function(req, res, next) {
        ...
        if (user && user.name === configSettings.basicAuthentication.username && user.pass === configSettings.basicAuthentication.password) {
            next();
        } else {
            res.statusCode = 401;
...
app.use('/proxy', require('./proxy')({
    ...
    basicAuthentication: configSettings.basicAuthentication
}));
