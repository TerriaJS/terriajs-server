...
function shortId(body, length) {
    ...
    var b = base62.encode(require('sha1')(buf));
    console.timeEnd('shortid');
    return b.slice(0, length || 16);
}
...
function saveS3(serviceOptions, body) {
    var params = {
        ...
        Key: shortId(body, serviceOptions.keyLength),
        Body: body
    };

    return S3(serviceOptions).putObject(params).promise()
        .then(function(result) {
            console.log('Saved key ' + params.Key + ' to S3 bucket ' + params.Bucket + '. Etag: ' + result.ETag);
            return params.Key;
        }).catch(function(e) {
            console.error(e);
            return e;
