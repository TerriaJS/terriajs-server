...
export default function (_hostName, port, options) {
  ...
  router.post("/", async (req, res) => {
    ...
    try {
      ...
      const forwardedPort = req.header("X-Forwarded-Port")?.replace(/^:/, "");
      const base = new URL(`${req.protocol}://${req.hostname}`);
      base.port = /^\d+$/.test(forwardedPort ?? "") ? forwardedPort : port;
      const resUrl = new URL(resPath, base).toString();
      res
        .location(resUrl)
        .status(201)
