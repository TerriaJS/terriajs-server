...
var url = require("url");
var { raw } = require("body-parser");
var rangeCheck = require("range_check");

var DO_NOT_PROXY_REGEX =
  /^(?:Host|X-Forwarded-Host|Proxy-Connection|Connection|Keep-Alive|Transfer-Encoding|TE|Trailer|Proxy-Authorization|Proxy-Authenticate|Upgrade|Expires|pragma|Strict-Transport-Security|Cookie|Authorization)$/i;
...
module.exports = function (options) {
  ...
  function proxyAllowedHost(host) {
    ...
    for (var i = 0; i < proxyDomains.length; i++) {
      ...
      if (host === domainLower || host.endsWith("." + domainLower)) {
        return true;
      }
    }
    return false;
  ...
  function doProxy(req, res, next, retryWithoutAuth, callback) {
    ...
    if (!protocolMatch || protocolMatch.length < 1) {
      ...
      if (remoteUrlString[matchedPart.length] !== "/") {
        remoteUrlString =
          matchedPart + "/" + remoteUrlString.substring(matchedPart.length);
      }
    }
    ...
    if (!proxyAllowedHost(remoteUrl.host)) {
      res
        .status(403)
        .send("Host is not in list of allowed hosts: " + remoteUrl.host);
      return;
    }
    ...
    if (filteredReqHeaders["x-forwarded-for"]) {
      filteredReqHeaders["x-forwarded-for"] =
        filteredReqHeaders["x-forwarded-for"] +
        ", " +
  ...
  function buildReqHandler(httpVerb) {
    function handler(req, res, next) {
      return doProxy(
        ...
        res,
        next,
        req.retryWithoutAuth,
        function (remoteUrl, filteredRequestHeaders, proxy, maxAgeSeconds) {
          try {
            var proxiedRequest = request({
              ...
              .on("socket", function (socket) {
                socket.once("lookup", function (err, address, family, host) {
                  if (rangeCheck.inRange(address, blacklistedAddresses)) {
                    res
                      .status(403)
                      .send("IP address is not allowed: " + address);
                    res.end();
                    proxiedRequest.abort();
                  }
                });
              ...
              .on("response", function (response) {
                ...
                // Skip caching if we have an error response
                maxAgeSeconds =
  ...
  router.get("/{*splat}", buildReqHandler("GET"));
  router.post(
    "/{*splat}",
    raw({
      type: function () {
