...
module.exports = function (options) {
  ...
  async function doProxy(req, res, next) {
    /**
    ...
    if (hostAuth && hostAuth.authorization) {
      authMethods.push({
        type: "proxy",
    ...
    if (authMethods.length > 0) {
      ...
      // No special auth methods, just use current headers
      authMethods.push(null);
    }
    ...
    async function tryRequest(authMethodIndex) {
      ...
      try {
        ...
        // On error, try next auth method if available
        if (authMethodIndex < authMethods.length - 1) {
          return tryRequest(authMethodIndex + 1);
        }
        throw err;
      }
