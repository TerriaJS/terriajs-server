...
module.exports = function (options) {
  ...
  async function doProxy(req, res, next, retryWithoutAuth) {
    /**
    ...
    const remoteUrl = url.parse(target);
    ...
    const filteredReqHeaders = filterHeaders(req.headers, req.connection);
    ...
    const targetUrl = url.format(remoteUrl);
    ...
    // Use undici request for proxying
    undici
      .request(targetUrl, {
      ...
      .then(async (response) => {
        console.log("Proxy response received for:", targetUrl);
        const { statusCode, headers, body } = response;

        // Check if we should retry without auth
        // if (
        ...
        //   // Try again without credentials in order to give the user the opportunity to supply their own.
        //   req.retryWithoutAuth = true;

        //   // Consume the body before retrying
        //   await body.dump();
        //   return doProxy(req, res, next, true);
        // }
  ...
  router.get(/^\/_([^/]+)(?:\/(.*))?$/, (req, res, next) => {
    ...
    return doProxy(req, res, next, req.retryWithoutAuth);
  });
