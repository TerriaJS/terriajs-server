 ...
 * @param {String} options.bypassUpstreamProxyHosts A comma separated list of hosts that will bypass the specified
 *                      upstream_proxy, e.g. "lanhost1,lanhost2"
 *
...
module.exports = function(options) {
    ...
    function doProxy(req, res, next, callback) {
        ...
        var proxy;
        if (options.upstreamProxy && !(remoteUrl.host in options.bypassUpstreamProxyHosts)) {
...
function processHeaders(headers) {
    ...
    result['Cache-Control'] = 'public,max-age=' + DEFAULT_MAX_AGE_SECONDS;
