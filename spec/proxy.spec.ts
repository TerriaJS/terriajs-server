  ...
  describe("on post,", function() {
    ...
    it("should pass the body through", function(done) {
      request(buildApp(openProxyOptions))
        ...
        .expect(200)
        .expect(function() {
          expect(fakeRequest.calls.argsFor(0)[0].body).toEqual(
            new Buffer("boaty mcboatface")
          );
        })
        .end(assert(done));
    });
  ...
  function doCommonTests(verb: string) {
    ...
    describe("should change headers", function() {
      ...
      it("to filter out disallowed ones passed in req", function(done) {
        request(buildApp(openProxyOptions))
          ...
          .expect(function() {
            expect(
              fakeRequest.calls.argsFor(0)[0].headers["Proxy-Connection"]
            ).toBeUndefined();
            expect(
              fakeRequest.calls.argsFor(0)[0].headers["unfilteredheader"]
            ).toBe("don't delete me!");
          })
          .end(assert(done));
      });
    ...
    describe("upstream proxy", function() {
      it("is used when one is specified", function(done) {
        request(
          buildApp({ upstreamProxy: "http://proxy/", proxyAllDomains: true })
        )
    ...
    describe("when domain has basic authentication specified", function() {
      it("should set an auth header for that domain", function(done) {
        request(
          ...
          .expect(200)
          .expect(function() {
            expect(fakeRequest.calls.argsFor(0)[0].headers.authorization).toBe(
              "blahfaceauth"
            );
            expect(fakeRequest.calls.argsFor(0)[0].method).toBe(verb);
          })
      ...
      it("should not set auth headers for other domains", function(done) {
        request(
          ...
          .expect(function() {
            expect(
              fakeRequest.calls.argsFor(0)[0].headers.authorization
            ).toBeUndefined();
            expect(fakeRequest.calls.argsFor(0)[0].method).toBe(verb);
          })
          .end(assert(done));
    ...
    describe("when domain has other headers specified", function() {
      it("should set the header for that domain", function(done) {
        request(
          ...
          .expect(200)
          .expect(function() {
            expect(fakeRequest.calls.argsFor(0)[0].headers["Secret-Key"]).toBe(
              "ABCDE12345"
            );
            expect(
              fakeRequest.calls.argsFor(0)[0].headers["Another-Header"]
            ).toBe("XYZ");
            expect(fakeRequest.calls.argsFor(0)[0].method).toBe(verb);
          })
          .end(assert(done));
      ...
      it("should not set the headers for other domains", function(done) {
        request(
          ...
          .expect(function() {
            expect(
              fakeRequest.calls.argsFor(0)[0].headers["Secret-Key"]
            ).toBeUndefined();
            expect(
              fakeRequest.calls.argsFor(0)[0].headers["Another-Header"]
            ).toBeUndefined();
