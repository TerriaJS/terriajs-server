...
function doCommonTest(methodName) {
  ...
  describe("stream error handling", () => {
    ...
    it("should handle stream errors when response is being piped", async () => {
      const { Readable } = require("stream");

      // Create a stream that will error during transmission
      testServer.addRoute(methodName, "/stream-error", (req, res) => {
        ...
        const errorStream = new Readable({
          read() {
            if (!dataSent) {
              ...
              this.push("Starting data...");
              dataSent = true;

              // Then error after a short delay
              setTimeout(() => {
        ...
        // Handle errors in test server too
        errorStream.on("error", (err) => {
          console.log("Test server stream error (expected):", err.message);
          // When stream errors, properly end the response
        ...
        res.on("error", (err) => {
          console.log("Test server response error (expected):", err.message);
        });
      ...
      // The request should handle the stream error gracefully
      // Either return an error or partial response, but not crash
      const response = await supertestReq(app)[methodName](
        `/proxy/localhost:${TEST_SERVER_PORT}/stream-error`
      );

      // We don't expect 200 here since the stream failed
    ...
    it("should handle client disconnect during streaming", async () => {
      ...
      testServer.addRoute(methodName, "/slow-stream", (req, res) => {
        res.set("Content-Type", "text/plain");
        res.status(200);
      ...
      // Start the request but abort it mid-stream
      ...
      try {
        await supertestReq(app)
          [methodName](`/proxy/localhost:${TEST_SERVER_PORT}/slow-stream`)
          .timeout(200);
        // .signal(controller.signal);

        // If we get here, the request completed before abort
        // That's fine, test still validates abort handling
      } catch (err) {
        console.log(err);
        // Expected - either timeout or abort
        expect(
          err.code === "ABORT_ERR" || err.code === "ECONNRESET" || err.timeout
        ).toBeTruthy();
      }

      // Important: Server should still be responsive after client disconnect
      testServer.addRoute(methodName, "/health", (req, res) => {
        res.json({ status: "ok" });
      });

      const healthCheck = await supertestReq(app)
        [methodName](`/proxy/localhost:${TEST_SERVER_PORT}/health`)
        .expect(200);

      expect(healthCheck.body.status).toBe("ok");
    });

    it("should handle upstream connection reset during streaming", async () => {
      const net = require("net");
      let server;
      let serverPort;

      // Create a raw TCP server that will close the connection mid-stream
      await new Promise((resolve) => {
        server = net.createServer((socket) => {
          ...
          // Send a chunk
          socket.write("10\r\n"); // Chunk size in hex (16 bytes)
          socket.write("Initial chunk..\r\n");
          socket.write("\r\n");

          // Close connection abruptly without proper termination
          setTimeout(() => {
            socket.destroy(); // Simulate connection reset
          }, 50);
        });
      ...
      try {
        // Request should handle the abrupt connection close
        await supertestReq(app)
          [methodName](`/proxy/localhost:${serverPort}/`)
          .timeout(2000);

        // May or may not fail depending on timing
        // Either way, server should handle it gracefully
      } catch (err) {
        // Connection errors are expected
        expect(err.code === "ECONNRESET" || err.timeout).toBeTruthy();
      } finally {
        // Clean up
        server.close();
      }
    ...
    it("should handle large response streams without memory issues", async () => {
      const { Readable } = require("stream");

      // Create endpoint that sends large amount of data
      testServer.addRoute(methodName, "/large-stream", (req, res) => {
        res.set("Content-Type", "text/plain");
        res.status(200);
          ...
          read() {
            if (sent < targetSize) {
              const chunk = Buffer.alloc(
                Math.min(chunkSize, targetSize - sent),
                "a"
              );
              this.push(chunk);
              sent += chunk.length;
      ...
      const response = await supertestReq(app)
        [methodName](`/proxy/localhost:${TEST_SERVER_PORT}/large-stream`)
        .expect(200)
        .timeout(10000);
      ...
      // Verify data was received (text response uses .text property)
      const receivedData = response.text || response.body;
      expect(receivedData.length).toBeGreaterThan(1024 * 1024);

      // Memory increase should be reasonable (not holding entire response in memory)
      // Allow up to 50MB increase for buffering, V8 overhead, etc.
      expect(memoryIncrease).toBeLessThan(50 * 1024 * 1024);
    });

    it("should handle backpressure correctly during streaming", async () => {
      const { Readable } = require("stream");

      // Create a fast producer
      testServer.addRoute(methodName, "/fast-stream", (req, res) => {
        ...
        const fastStream = new Readable({
          read() {
            // Produce data as fast as possible
            while (chunks < maxChunks) {
              const canPush = this.push(Buffer.alloc(chunkSize, chunks % 256));
              chunks++;

              if (!canPush) {
                // Stream is applying backpressure, stop for now
                break;
              }
            }
      ...
      const response = await supertestReq(app)
        [methodName](`/proxy/localhost:${TEST_SERVER_PORT}/fast-stream`)
        .expect(200)
        .timeout(5000);

      // Verify all data received correctly
      expect(response.body.length).toBe(1000 * 1024);
    });
  });
}
